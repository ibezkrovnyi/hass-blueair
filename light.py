...
async def async_setup_entry(hass, config_entry, async_add_entities):
    """Set up the Blueair backlights from config entry."""
...
class BlueairLight(BlueairEntity, LightEntity):
    ...
    def supported_color_modes(self) -> set[ColorMode]:
        ...
        if self.coordinator.brightness is not None:
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        await self.coordinator.set_brightness(kwargs.get(ATTR_BRIGHTNESS, 255))

    async def async_turn_off(self, **kwargs: Any) -> None:
        """Turn off or control the light."""
        await self.coordinator.set_brightness(kwargs.get(ATTR_BRIGHTNESS, 0))
